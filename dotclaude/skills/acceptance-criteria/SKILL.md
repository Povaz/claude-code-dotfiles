...
- **Standalone vs. orchestrated.** When called via `/anchored-specs`, the orchestrator passes the next free `Y` for the parent story. When called standalone, scan `acceptance-criteria.md` for the highest existing Y under this X and pick the next; if no AC exists yet for this story, start at Y=1.
