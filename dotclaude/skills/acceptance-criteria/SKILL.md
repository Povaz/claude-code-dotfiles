...
- **Standalone vs. host-process.** If a host process supplies the next free `Y` for the parent story, use it. Otherwise scan `acceptance-criteria.md` for the highest existing Y under this X and pick the next; if no AC exists yet for this story, start at Y=1.
